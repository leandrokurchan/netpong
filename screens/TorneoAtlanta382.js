...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=+541165991011${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoAtlanta382({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Lugar source={require("../assets/mesasAtlanta.jpg")}/>
          ...
          <Categorias>
            ...
            <Informacion>
                <Text>7ma 10hs</Text>
               <Text>{"\n"}</Text>
               <Text>6ta 12:30hs</Text>
               <Text>{"\n"}</Text>
               <Text>5ta 15hs</Text>
               <Text>{"\n"}</Text>
               <Text>4ta 17:30hs</Text>
            </Informacion>
          </Categorias>
          <Cupos>
          ...
          <Precio>
            ...
            <Informacion> 
                ...
                <Text>{"\n"}</Text>
                <Text>1000 socios</Text>
                <Text>{"\n"}</Text>
                <Text>700 segunda insc.</Text>
            </Informacion>
          </Precio>
          <Mesas>
          ...
          <Organizador>
            ...
            <Informacion>Gonzalo Tiercin</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/541165991011?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/541165991011";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
