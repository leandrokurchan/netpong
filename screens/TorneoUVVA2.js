...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=1158711614${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoUVVA2({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Lugar source={require("../assets/mesasUVVA.jpg")}/>
          ...
          <Precio>
            ...
            <Informacion> 
                <Text>1300 general</Text>
                <Text>{"\n"}</Text>
                <Text>1100 segunda insc.</Text>
                <Text>{"\n"}</Text>
                <Text>1100 socios</Text>
                <Text>{"\n"}</Text>
                <Text>1100 damas y menores</Text>
            </Informacion>
          </Precio>
          <Mesas>
            <Imagen source={require("../assets/Mesas.png")}/>
            <Subtitle>Mesas</Subtitle>
            <Informacion>6 mesas Almar</Informacion>
          </Mesas>
          <Adaptaciones>
          ...
          <Organizador>
            ...
            <Informacion>Rodrigo Apaza</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/1158711614?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/1158711614";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
