...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=11623-99247${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoRioja130({ navigation }) {
    return (
      <Container>
  ...
  />
          ...
          <Precio>
            ...
            <Informacion> 
                ...
                <Text>{"\n"}</Text>
                <Text>900 segunda insc.</Text>
            </Informacion>
          </Precio>
          <Mesas>
            <Imagen source={require("../assets/Mesas.png")}/>
            <Subtitle>Mesas</Subtitle>
            <Informacion>7 mesas Almar</Informacion>
          </Mesas>
          <Adaptaciones>
          ...
          <Organizador>
            ...
            <Informacion>Thiago Espina De La Fuente</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/11623-99247?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/11623-99247";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
