...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=+541154813976${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoCasaPadua16({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Lugar source={require("../assets/mesasCasaPadua.jpg")}/>
          ...
          <Mesas>
            ...
            <Informacion>4 mesas</Informacion>
          </Mesas>
          <Adaptaciones>
          ...
          <Organizador>
            ...
            <Informacion>Jorge Rodrigo Balcaza</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/1154813976?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/1154813976";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
