...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=11-2154-8240${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoZenit53({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Lugar source={require("../assets/mesasZenit.jpg")}/>
          ...
          <Categorias>
            ...
            <Informacion>
               <Text>6ta 10:30hs</Text>
               <Text>{"\n"}</Text>
               <Text>5ta 13hs</Text>
               <Text>4ta 15:30hs</Text>
               <Text>{"\n"}</Text>
               <Text>3ra 18hs</Text>
            </Informacion>
          </Categorias>
          ...
          <Organizador>
            ...
            <Informacion>Martin Lima</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/11-2154-8240?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/11-2154-8240";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
