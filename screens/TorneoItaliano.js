...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=112580-4030${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoItaliano67({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Organizador>
            ...
            <Informacion>Sebastián Lacuadra</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/112580-4030?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/112580-4030";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
