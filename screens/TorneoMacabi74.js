...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=+1161560017${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoMacabi74({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Lugar source={require("../assets/mesasMacabi.jpg")}/>
          ...
          <Categorias>
            ...
            <Informacion>
               <Text>3ra 10hs</Text>
               <Text>{"\n"}</Text>
               <Text>2da 12:30hs</Text>
               <Text>{"\n"}</Text>
               <Text>1ra 15:30hs</Text>
            </Informacion>
          </Categorias>
          <Cupos>
          ...
          <Precio>
            ...
            <Informacion> 
                <Text>1500 general</Text>
                <Text>{"\n"}</Text>
               <Text>1300 segunda insc.</Text>
               <Text>{"\n"}</Text>
               <Text>1300 socios</Text>
            </Informacion>
          </Precio>
          <Mesas>
            <Imagen source={require("../assets/Mesas.png")}/>
            <Subtitle>Mesas</Subtitle>
            <Informacion>6 mesas</Informacion>
          </Mesas>
          <Adaptaciones>
          ...
          <Organizador>
            ...
            <Informacion>Ezequiel López Márquez</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/1161560017?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/1161560017";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
