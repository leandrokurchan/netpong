...
handleWhatsAppButton = (whatsappNumber) => {
  ...
  const whatsappUrl = `whatsapp://send?phone=+1134947531${whatsappNumber}&text=${encodeURIComponent(message)}`
...
export default function TorneoSocialClub14({ navigation }) {
    return (
      <Container>
...
>
  ...
  />
          ...
          <Mesas>
            ...
            <Informacion>7 mesas</Informacion>
          </Mesas>
          <Adaptaciones>
          ...
          <Organizador>
            ...
            <Informacion>Nicolás Gomez</Informacion>
          </Organizador>
          ...
          <TouchableOpacity onPress={() => {
            ...
            const url = `https://wa.me/1134947531?text=${encodeURIComponent(message)}`;
...
<View style={{ backgroundColor: '#00B116', marginTop: 295, borderRadius: 10, width: 250, height: 80, alignSelf: 'center' }}>
  ...
  <Button 
    ...
    color="white"
    onPress={() => {
      const url = "https://wa.me/1134947531";
      Linking.canOpenURL(url).then(supported => {
        if (!supported) {
          console.log("Can't handle url: " + url);
        } else {
